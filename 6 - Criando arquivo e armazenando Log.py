...
# Define o caminho raiz para o arquivo de log (pode ajustar conforme necessário)
...
# Decorador para log em arquivo e console
def log_transacao(func):
    def envelope(*args, **kwargs):
        resultado = func(*args, **kwargs)
        data_hora = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
        log_msg = f"[{data_hora}] Função '{func.__name__}' executada com args={args} kwargs={kwargs}. Resultado: {resultado}\n"
...
def depositar(clientes):
    ...
    try:
        ...
        print("\n--- Valor inválido! Informe um número válido. ---")
        return
...
def exibir_extrato(clientes):
    ...
    if not transacoes:
        ...
        print(f"{'Data':<19} | {'Tipo':<10} | {'Valor':>14}")
        print("-" * 49)
        for t in transacoes:
            # Espaçamento e alinhamento com R$ e 2 espaços
            print(f"{t['data']:<19} | {t['tipo']:<10} | {'R$  ' + format(t['valor'], ',.2f'):>14}")

    print(f"\n{'SALDO':<19}  {'':12}  {'R$  ' + format(conta.saldo, ',.2f'):>14}\n")
    print("=================================================\n")
